# Actual iteration-1 board (Chapters 3 and 4)

Start date: 2026-02-16  
End date: 2026-03-08  

Checklist:
1. GitHub entry timestamps
2. User stories are correct (see textbook guidance)

* Assumed Velocity: 0.7
* Number of developers: 1
* Total estimated amount of work: 10 days

## Task tracking (Practical 4)
- Task board: [docs/iteration_1_tasks.md](./docs/iteration_1_tasks.md)

User stories or tasks:
1. [Browse restaurants](./user_stories/user_story_01_browse_restaurants.md), priority 10, 1 day
2. [View restaurant menu](./user_stories/user_story_02_view_menu.md), priority 10, 1 day
3. [Add items to cart](./user_stories/user_story_03_add_to_cart.md), priority 10, 1 day
4. [Update cart quantities](./user_stories/user_story_04_update_cart.md), priority 20, 1 day
5. [Place an order](./user_stories/user_story_05_place_order.md), priority 10, 2 days
6. [Restaurant manages menu](./user_stories/user_story_06_manage_menu.md), priority 20, 2 days
7. [Restaurant views incoming orders](./user_stories/user_story_07_view_orders.md), priority 10, 1 day
8. [Restaurant updates order status](./user_stories/user_story_08_update_order_status.md), priority 20, 1 day

In progress:
(none)

Completed:
* US-01 Browse restaurants (J)
* US-02 View restaurant menu (J)
* US-03 Add items to cart (J), 2026-03-04

### Burn Down for iteration-1 (update at least once per week)
* 4 weeks left, 10 days of estimated amount of work
* 2 weeks left, __ days
* 1 week left, __ days
* 0 weeks left, 0 days
* Actual Velocity: __

